#!/usr/bin/env python3
"""SSTRF Phase V scoring pipeline (pre-reg §2–§3, §7.1)."""
...
from sstrf_scoring_evidence import (
    ...
    build_rater_payload,
    build_stage1_escalation_packets,
    build_tier2_drift_packets,
    load_phase_v_manifest,
    load_trial_label_map,
    save_trial_label_map,
    trial_record_from_label,
)
from mirofish_backend.llm.model_profiles import OPENAI_DEFAULT_ID

from sstrf_scoring_judge import (
    INSTRUCTIONS_VERBATIM,
    JUDGE_CONTRADICTION_GUIDANCE_LOCATION,
    JUDGE_PRIMARY_MODEL,
    JUDGE_SYSTEM_PROMPT,
...
CALIBRATION_GATE_RESULT = SCORING_DIR / "calibration_gate_result.json"
HUMAN_REVIEW_QUEUE = SCORING_DIR / "human_review_queue.json"
SCORING_STATE = SCORING_DIR / "scoring_state.json"
JUDGE_SCORING_SUMMARY = SCORING_DIR / "phase_v_judge_scoring_summary.json"
TIER1_PLAUSIBILITY_SUMMARY = SCORING_DIR / "tier1_plausibility_summary.json"
TIER2_DRIFT_PACKETS = SCORING_DIR / "tier2_drift_packets.json"
STAGE1_ESCALATION_PACKETS = SCORING_DIR / "stage1_escalation_packets.json"
...
def _write_calibration_gate_result(results: dict[str, Any]) -> None:
    ...
    CALIBRATION_GATE_RESULT.write_text(json.dumps(results, indent=2), encoding="utf-8")
...
def _save_scoring_state(state: dict[str, Any]) -> None:
    ...
    SCORING_STATE.write_text(json.dumps(state, indent=2), encoding="utf-8")
...
async def cmd_dry_run(args: argparse.Namespace) -> int:
    manifest = load_phase_v_manifest(Path(args.manifest) if args.manifest else None)
    mapping = load_trial_label_map()
    if not mapping:
        mapping = assign_trial_labels(accepted_trials(manifest), assignment_seed=args.assignment_seed)
        if args.write_label_map:
            save_trial_label_map(mapping)
    label = args.trial_label or next(iter(mapping))
...
async def cmd_run_calibration(args: argparse.Namespace) -> int:
    ...
    results: dict[str, Any] = {
        "run_at": _utc_now(),
        "pass_1_seed": pass_1_seed,
        "pass_2_seed": pass_2_seed,
        "prompt_config": {
            "system_prompt": JUDGE_SYSTEM_PROMPT,
            "contradiction_guidance_location": JUDGE_CONTRADICTION_GUIDANCE_LOCATION,
        },
        "attempts": [],
    }
    ...
    for spec in chain:
        try:
            p1 = await run_calibration_pass(pass_seed=pass_1_seed, profile_spec=spec)
            p2 = await run_calibration_pass(pass_seed=pass_2_seed, profile_spec=spec)
        ...
        if _is_primary_openai_judge(spec):
            print(
                ...
                "(human decision required per pre-reg v1.2 amendment 3)"
            )
            break
    _finalize_calibration_results(results, passed, chosen_model, chosen_profile_id)
    _write_calibration_gate_result(results)
...
async def cmd_score_all_trials(args: argparse.Namespace) -> int:
    ...
    manifest = load_phase_v_manifest()
    mapping = load_trial_label_map()
    if not mapping:
        mapping = assign_trial_labels(accepted_trials(manifest), assignment_seed=args.assignment_seed)
        save_trial_label_map(mapping)
    ...
    for label in sorted(mapping):
        ...
        if args.execute_judge_calls:
            ...
            if not args.verbose:
                print(f"scored {label} pass1={pass_1.scores} pass2={pass_2.scores}")
    ...
    if args.execute_judge_calls and state.get("trials"):
        ...
        summary = {
            ...
            "manifest_path": str(SCORING_MANIFEST),
            "note": "Full manifest written at --finalize; this summary is post judge scoring only.",
        }
        ...
        SCORING_DIR.mkdir(parents=True, exist_ok=True)
        JUDGE_SCORING_SUMMARY.write_text(json.dumps(summary, indent=2), encoding="utf-8")
        print(
        ...
        print(f"auto_resolved_diff_of_one cells: {len(diff_of_one)}")
        print(f"summary written to {JUDGE_SCORING_SUMMARY}")
    _save_scoring_state(state)
    return 0
...
async def cmd_prepare_adjudication(args: argparse.Namespace) -> int:
    ...
    SCORING_DIR.mkdir(parents=True, exist_ok=True)
    TIER1_PLAUSIBILITY_SUMMARY.write_text(json.dumps(tier1, indent=2), encoding="utf-8")
    ...
    if args.drift_sampling_mode == "trial" and sampled_trials:
        mapping = load_trial_label_map()
        manifest = load_phase_v_manifest()
        tier2 = build_tier2_drift_packets(
            sampled_trials,
            mapping=mapping,
            manifest=manifest,
        )
        TIER2_DRIFT_PACKETS.write_text(json.dumps(tier2, indent=2), encoding="utf-8")

    stage1_empty = await build_stage1_escalation_packets(
        [],
        mapping=load_trial_label_map(),
        manifest=load_phase_v_manifest(),
        sqlite_path=Path(args.sqlite),
    )
    STAGE1_ESCALATION_PACKETS.write_text(json.dumps(stage1_empty, indent=2), encoding="utf-8")
    ...
    HUMAN_REVIEW_QUEUE.write_text(json.dumps(queue, indent=2), encoding="utf-8")
    _save_scoring_state(state)
    print(
    ...
    if tier2 is not None:
        print(f"tier2 packets: {TIER2_DRIFT_PACKETS}")
    print(f"stage1 escalation scaffold: {STAGE1_ESCALATION_PACKETS}")
    return 0
...
def cmd_finalize(args: argparse.Namespace) -> int:
    ...
    for label in sorted(state.get("trials") or {}):
        ...
        for pid in required:
            ...
            human_count += 1
        final_scores = {pid: int(adjudicated[pid]) for pid in adjudicated if adjudicated[pid] is not None}
        trials_state[label] = {
            **trial_work,
    ...
    if not drift_sample:
        raise RuntimeError(
            "drift_sample missing — run --prepare-adjudication before --finalize"
        )
    ...
    if len(imported_drift) != len(drift_sample):
        raise RuntimeError(
            ...
            f"size {len(drift_sample)} — import Mark's drift-check scores for every "
            "sampled cell before --finalize"
        )
    ...
    SCORING_DIR.mkdir(parents=True, exist_ok=True)
    SCORING_MANIFEST.write_text(json.dumps(manifest_out, indent=2), encoding="utf-8")
    print(
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="SSTRF Phase V scoring pipeline")
    ...
    p.add_argument("--assignment-seed", type=int, default=42)
    ...
    p.add_argument(
        "--drift-sample-seed",
        type=int,
        default=DRIFT_CELL_SAMPLE_SEED,
        help="Cell-level drift sample seed (legacy v1.4; use with --drift-sampling-mode cell)",
    )
    p.add_argument(
        "--drift-trial-sample-seed",
        type=int,
        default=DRIFT_TRIAL_SAMPLE_SEED,
        help="Trial-level drift sample seed (pre-reg v1.5 default)",
    )
