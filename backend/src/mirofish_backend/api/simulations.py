...
@router.get("/simulations/{simulation_id}/memory-context-report")
async def get_simulation_memory_context_report(simulation_id: str) -> dict[str, Any]:
    """JSON summary of per-turn memory inclusion/exclusion (senna-iter-45)."""
    settings = get_settings()
    ...
    if st in ("pending", "running"):
        raise HTTPException(
            status_code=409,
            detail="Simulation not finished; memory-context-report is available after the run completes or fails.",
        )
